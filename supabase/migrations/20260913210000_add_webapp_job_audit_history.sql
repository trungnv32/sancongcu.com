-- Audit snapshot for each paid Webapp generation. This preserves the exact
-- customer-facing selections used at the time of the request.
...
comment on column public.webapp_jobs.request_params is
  'Snapshot of Webapp input selections and prompt template for complaint review.';
...
begin
  ...
  if jsonb_typeof(coalesce(p_request_params, '{}'::jsonb)) <> 'object' then
  ...
  insert into public.webapp_jobs (
    user_id, skill_id, quoted_amount_vnd, input_paths, instruction, request_params
  ) values (
    p_user_id,
    p_skill_id,
    v_price,
    p_input_paths,
    left(coalesce(p_instruction, ''), 1000),
...
revoke all on function public.webapp_start_job(uuid, uuid, jsonb, text, integer, boolean, jsonb)
  from public, anon, authenticated;
grant execute on function public.webapp_start_job(uuid, uuid, jsonb, text, integer, boolean, jsonb)
  to service_role;
